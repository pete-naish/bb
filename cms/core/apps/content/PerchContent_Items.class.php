...
class PerchContent_Items extends PerchFactory
{
    ...
    public function create_new_revision($regionID, $old_rev, $new_rev, $copy_resources=false)
    {
        ...
        if ($copy_resources) {
            $sql = 'REPLACE INTO '.PERCH_DB_PREFIX.'content_resources (itemRowID, resourceID)
                    SELECT c2.itemRowID, cr.resourceID 
                    FROM '.PERCH_DB_PREFIX.'content_resources cr, '.PERCH_DB_PREFIX.'content_items c1, '.PERCH_DB_PREFIX.'content_items c2
                    WHERE  cr.itemRowID=c1.itemRowID AND c1.itemID = c2.itemID AND c1.regionID='.$this->db->pdb($regionID).' AND c2.regionID='.$this->db->pdb($regionID).'
    ...
    public function delete_old_revisions($regionID, $number_remaining)
    {
        ...
        $sql = 'DELETE FROM '.PERCH_DB_PREFIX.'content_resources WHERE itemRowID NOT IN 
